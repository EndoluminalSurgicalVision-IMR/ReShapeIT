...
def create_mesh_caselatent(model, filename, subject_idx=0, embedding=None, N=128, max_batch=64 ** 3, offset=None,
                           scale=None,
                           level=0.0, get_color=True):
    start = time.time()
    ply_filename = filename
    ...
    while head < num_samples:
        ...
        sample_subset = samples[head: min(head + max_batch, num_samples), 0:3].cuda()[None, ...]
        samples[head: min(head + max_batch, num_samples), 3] = (
            model.inference(sample_subset, embedding, subject_idx)
                .squeeze()  # .squeeze(1)
                .detach()
                .cpu()
        )
    ...
    if not get_color:
        ...
        convert_sdf_samples_with_color_to_ply_caselatent(
            model,
            embedding,
            sdf_values.data.cpu(),
            voxel_origin,
...
def get_mesh_color_caselatent(mesh_points, embedding, model, subject_idx):
    model.eval()
    ...
    while head < num_samples:
        ...
        mesh_colors[head: min(head + max_batch, num_samples), 0:3] = (
            model.get_template_coords(sample_subset, embedding, subject_idx)
                .squeeze()  # .squeeze(1)
                .detach()
                .cpu()
        )
