...
class SingleBVPNet(MetaModule):
    ...
    def forward(self, model_input, params=None, latent=None):
        ...
        ## TODO: Modify the original SingleBVPNet Here:
        if latent is not None:
            _, num_sample, _ = coords.shape
            latent = latent.unsqueeze(1)
            latent_input = latent.expand(-1, num_sample, -1)
            # various input processing methods for different applications
            output = self.net(torch.cat((coords_org, latent_input), dim=2), get_subdict(params, 'net'))
        else:
            # various input processing methods for different applications
            output = self.net(coords_org, get_subdict(params, 'net'))
...
class SingleBVPNetWOCC(MetaModule):
    '''A canonical representation network for a BVP.'''
    ...
    def forward(self, model_input, params=None):
        ...
        coords_org = model_input['coords'].requires_grad_(True)
        coords = coords_org
        intensities = model_input['intensities']
        # print(coords_org.shape)
        # print(intensities.shape)

        # various input processing methods for different applications
        # output = self.net(torch.cat((coords_org, intensities), dim=2), self.get_subdict(params, 'net'))
        output = self.net(torch.cat((coords_org, intensities), dim=2), get_subdict(params, 'net'))
        return {'model_in': coords_org, 'model_out': output}
