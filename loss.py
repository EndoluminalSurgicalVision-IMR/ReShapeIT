...
def ismm_train_loss(model_output, gt, loss_grad_deform=5, loss_grad_temp=1e2, loss_correct=1e2):
    gt_sdf = gt['sdf']
    ...
    template_code = model_output['template_code']
    deformation_template = model_output['deformation_template']
    correction_template = model_output['correction_template']

    latent_case = model_output['latent_case']
    latent_template = model_output['latent_template']
    ...
    template_code_constraint = torch.mean(template_code ** 2)

    latent_case_constraint = torch.mean(latent_case ** 2)
    latent_template_constraint = torch.mean(latent_template ** 2)

    deformation_template_constraint = (deformation_template - torch.zeros_like(deformation_template)) ** 2
    correction_template_constraint = (correction_template - torch.zeros_like(correction_template)) ** 2
    ...
    return {
        ...
        'template_code_constraint': template_code_constraint.mean() * 1e6,
        ...
        'correction_template_constraint': deformation_template_constraint.mean() * loss_correct,
        'deformation_template_constraint': correction_template_constraint.mean() * loss_correct,

        'latent_case': latent_case_constraint.mean() * 1e5,
        'latent_template': latent_template_constraint.mean() * 1e5,
    }



def ismm_finetune_loss(model_output, gt):
    gt_sdf = gt['sdf']
    gt_normals = gt['normals']
