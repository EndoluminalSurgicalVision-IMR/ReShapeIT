...
def finetune(model, train_dataloader, epochs, lr, steps_til_summary, epochs_til_checkpoint, model_dir,
             loss_schedules=None, **kwargs):
    print('Training Info:')
    ...
    with tqdm(total=len(train_dataloader) * epochs) as pbar:
        ...
        for epoch in range(epochs):
            ...
            for step, (model_input, gt) in enumerate(train_dataloader):
                ...
                losses = model(model_input, gt, is_Train=False)
                train_loss = 0.
...
if __name__ == '__main__':
    ...
    pretrained_dict = torch.load(meta_params['pretrain_path'])
    pretrained_update_dict = {key: value for key, value in pretrained_dict.items() if
                              (key in model_dict and 'latent_code' not in key)}
    model_dict.update(pretrained_update_dict)
    model.load_state_dict(model_dict)
    for name, module in model.named_children():
        if name in ['template_field']:
    ...
    finetune(model=model, train_dataloader=dataloader, model_dir=root_path, **meta_params)
